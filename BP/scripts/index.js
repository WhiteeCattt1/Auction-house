...
async function AuctionHouseUI(player, page = 1) {
    ...
    try {
        ...
        overworld.runCommand(`tellraw "${data.seller}" {"rawtext":[{"text":"${config.prefix}Player §g${player.name}§r purchased item §gx${item.amount}§r ${item.nameTag ?? `§g${item.typeId?.replace("minecraft:", "")?.replaceAll("_", " ")}`}§r from you for §a$${formatNumber(data.price)}§r!"}]}`)
        overworld.runCommand(`playsound random.fizz "${data.seller}"`)
        player.runCommand(`scoreboard players remove @s ${config.money_object} ${data.price}`)
